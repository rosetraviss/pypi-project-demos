...
# Try to mock the astro_mmdc imports since pydantic-core fails to load in workers.
# Instead of doing that, we will directly rely on `js_fetch` to proxy requests to the real API
# since Pyodide workers do not support loading the pydantic-core C extension correctly locally.
# However, we must ensure we include the original package to fulfill the requirement:
